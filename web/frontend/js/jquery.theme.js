...
$(document).ready(function(){
	...
	// 	$('#feature-image').attr('src', $(this).attr('href'));
	...
	// });
	 ...
	 if ($("#feature-image").length) {
		$("#feature-image").elevateZoom({
			zoomType : "inner",
			gallery:'gallery', 
			cursor: 'url(//cdn.shopify.com/s/files/1/0044/8732/t/2/assets/ico-zoom.png?1501),url(//cdn.shopify.com/s/files/1/0044/8732/t/2/assets/ico-zoom.cur?1501),auto', 
			galleryActiveClass: 'active', 
			imageCrossfade: true, 
			loadingIcon: '//cdn.shopify.com/s/files/1/0044/8732/t/2/assets/loader.gif?1501'
		});
	}  
		...
		if(cart.item_count > 0) {
			...
			jQ.each(cart.items, function(index, cartItem) {
				...
				if (index == (cart.items.length-1)) {
					$("#drawer .spinner").remove();
					$("#drawer .container").prepend('<form action="/cart" method="post">'+drawer+'<div class="actions"><input type="submit" name="checkout" value="Check Out" /></div></form>').hide().fadeIn(500);
				}
			});
  ...
  function windowResize() {
    ...
    if (window.innerWidth < 768) {
      $(".cart.mobile").find(".count").attr("id", "cart-target")
      $(".cart.desktop").find(".count").attr("id", "not-active")
    } else {
      $(".cart.desktop").find(".count").attr("id", "cart-target")
      $(".cart.mobile").find(".count").attr("id", "not-active")
    }
